...
import { MapPin, ExternalLink, Calendar, Clock, Sparkles, VolumeX, Upload, Play, Pause, Video } from 'lucide-react';
...
function getDB(): Promise<IDBDatabase | null> {
  return new Promise((resolve) => {
    try {
      const idxdb = window.indexedDB || (window as any).mozIndexedDB || (window as any).webkitIndexedDB || (window as any).msIndexedDB;
      if (!idxdb) return resolve(null);
      
      const request = idxdb.open('SangeetVideoDB', 1);
...
async function saveVideoToLocal(file: File): Promise<boolean> {
  try {
    ...
    return new Promise((resolve) => {
      ...
      const req = store.put(file, 'sangeet');
...
async function loadVideoFromLocal(): Promise<File | null> {
  try {
    const db = await getDB();
    if (!db) return null;
    return new Promise((resolve) => {
...
function SangeetVideo({ isPlaying }: SVGProps) {
  ...
  // Load custom video on mount
  useEffect(() => {
    const init = async () => {
      const savedFile = await loadVideoFromLocal();
      if (savedFile) {
        const url = URL.createObjectURL(savedFile);
        setVideoUrl(url);
        setIsCustom(true);
      } else {
        // Fallback to high-quality beautiful looping romantic wedding dance video
        setVideoUrl('https://assets.mixkit.co/videos/preview/mixkit-beautiful-wedding-couple-dancing-under-fairy-lights-33824-large.mp4');
        setIsCustom(false);
      }
      setLoading(false);
    };
  ...
  // Sync isPlaying state with global audio/event state
  useEffect(() => {
  ...
  const handleRemove = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      if (db) {
        const tx = db.transaction('videos', 'readwrite');
        tx.objectStore('videos').delete('sangeet');
      }
    } catch(err){}
    setVideoUrl('https://assets.mixkit.co/videos/preview/mixkit-beautiful-wedding-couple-dancing-under-fairy-lights-33824-large.mp4');
    setIsCustom(false);
  ...
  return (
    <div className="relative overflow-hidden w-full h-56 rounded-t-3xl bg-[#0a1420] border-b border-[#c6d7e5] flex flex-col items-center justify-center group">
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center gap-2">
          <div className="w-5 h-5 border-2 border-[#eab561] border-t-transparent rounded-full animate-spin" />
          <span className="text-[9px] text-[#ffecb3] uppercase tracking-widest font-sans">Preparing Sangeet Video...</span>
        </div>
      ) : (
        <>
          <video
          ...
          {/* Sangeet Text Badges Overlay */}
          <div className="absolute top-3 left-1/2 -translate-x-1/2 flex flex-col items-center pointer-events-none">
            <div className="bg-[#ffecb3]/95 px-5 py-1 rounded-full border border-[#eab561]/30 shadow-md">
              <span className="font-serif text-[10px] font-bold tracking-[0.25em] text-[#511316] uppercase">
                {isCustom ? "Sangeet Invitation" : "Sangeet Ceremony"}
              </span>
            </div>
            {!isCustom && (
              <span className="text-[8px] text-[#ffecb3] mt-1 font-sans uppercase tracking-[0.15em] opacity-80 filter drop-shadow">
                Demo Video
              </span>
            )}
          </div>
          ...
          {/* Hover Controls bar */}
          <div className="absolute bottom-3 left-0 right-0 px-3.5 flex justify-between items-center z-10 transition-all duration-300 transform translate-y-2 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 flex-wrap gap-2">
            <button
              onClick={triggerUpload}
              className="flex items-center gap-1 bg-[#eab561] hover:bg-[#d8a14d] text-[#2c0b0d] font-sans font-bold text-[9px] uppercase tracking-wider py-1 px-2.5 rounded-full shadow-lg border border-[#ffd596]/40 transition-all active:scale-95"
            >
              <Upload className="w-3 h-3" />
              {isCustom ? "Change Video" : "Upload Custom Video"}
            </button>

            {isCustom && (
              <button
                onClick={handleRemove}
                className="bg-red-950/80 hover:bg-red-900 border border-red-500/30 text-amber-100 font-sans text-[9px] py-1 px-2.5 rounded-full uppercase tracking-wider transition-all active:scale-95"
              >
                Reset Default
              </button>
            )}
            ...
            {!isCustom && (
              <span className="text-[8px] text-amber-200/90 font-mono italic">
                Choose video to preview live!
              </span>
            )}
          </div>

          {/* High-visibility Pulsating Indicator inside default player */}
          {!isCustom && (
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
              <div 
                onClick={triggerUpload}
                className="pointer-events-auto cursor-pointer flex flex-col items-center gap-1.5 bg-black/40 hover:bg-black/55 active:scale-95 transition-all text-white py-2 px-3.5 rounded-2xl border border-white/20 backdrop-blur-md shadow-2xl animate-pulse"
              >
                <Video className="w-4.5 h-4.5 text-[#ffecb3]" />
                <span className="font-sans text-[9px] font-bold tracking-widest uppercase text-amber-100 text-center">
                  ✦ Click to Add Your Video ✦
                </span>
                <span className="font-sans text-[7px] text-gray-300/80 text-center uppercase tracking-wider">
                  (Muted playback • Saved locally)
                </span>
              </div>
            </div>
          )}

          {/* Safe loading progress ring */}
          {uploadProgress && (
            <div className="absolute inset-0 bg-black/80 flex flex-col items-center justify-center gap-1.5 z-20">
              <div className="w-6 h-6 border-2 border-[#eab561] border-t-transparent rounded-full animate-spin" />
              <p className="text-[#ffecb3] text-[9px] uppercase tracking-widest font-sans font-bold">Storing safely in browser...</p>
            </div>
          )}
        </>
      )}
          ...
          <div key={item.day} className="flex flex-col gap-3">
            ...
            >
              ...
              <div className="relative">
                ...
                {item.illustration === 'sangeet' && <SangeetVideo isPlaying={isGlobalPlaying} />}
